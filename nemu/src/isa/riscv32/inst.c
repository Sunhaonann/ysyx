...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {//根据不同的指令类型，从指令中提取出对应的字段值，并将其存储在相应的变量中
...
INSTPAT_START();
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = SEXT(src1, 32) / SEXT(src2, 32));//BITS
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
	...
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, uint32_t a = src1; uint32_t b = src2; uint64_t tmp = (uint64_t)a * (uint64_t)b; R(rd) = BITS(tmp, 63, 32));
  ...
  //xian kuozhan zai %
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));// SEXT (imm)
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + SEXT(imm,32), 8, src2));//8//
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = (sword_t)SEXT(src1, 32) >> BITS(src2, 4, 0));//diff
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = SEXT(src1, 32) >> BITS(src2, 4, 0));// a little
