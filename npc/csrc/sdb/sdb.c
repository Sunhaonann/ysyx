...
static int cmd_x(char *args){
	...
	int N = atoi(p_N);
	if (N <= 0){puts(x_help); return 0;}
	char *p_EXPR == strtok(NULL," ");
	if (p_EXPR == NULL) { puts(x_help); return 0; }
	long long addr = atoll(p_EXPR);
	if (addr <= 0) { puts(x_help); return 0;}
//输出的格式为：内存地址（8位十六进制）和4个字节的内容（每个字节2位十六进制）
  for(int i =0;i < N; i++){
...
void sdb_mainloop() {
  ...
  printf(ANSI_FG_GREEN
         ...
         "q    : Exit NEMU"                                                
         "si   : Single -step exection"                                   
      	 "info : Show registers or watchpoint"                        
      	 "x    : x N EXPR, Scan memory"                                     
      	 "p    : Print expression"                                          
         "w    : Set watchpoint"                                            
      	 "d    : Deleteb watchpoint"                                
	ANSI_NONE);
